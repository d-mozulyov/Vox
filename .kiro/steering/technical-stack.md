...
- **Windows** (x64, arm64)
- **Linux** (x64, arm64) — статические бинарники (CGO_ENABLED=0)
- **macOS** (x64, arm64)
...
- **Предпочтение pure Go библиотекам** (без CGO)

**Текущие зависимости:**
- `github.com/ebitengine/oto/v3` — аудио (pure Go, без CGO)
- `golang.design/x/hotkey` — горячие клавиши
- `github.com/getlantern/systray` — системный трей

### Сборка и релизы

- Все бинарники собираются как **статические** (CGO_ENABLED=0)
